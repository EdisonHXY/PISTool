...
应用程序向导已为您创建了此 TestTools 应用程序。此应用程序不仅演示 Microsoft 基础类的基本使用方法，还可作为您编写应用程序的起点。
...
TestTools.h
    这是应用程序的主头文件。
    其中包括其他项目特定的标头（包括 Resource.h），并声明 CTestToolsApp 应用程序类。

TestTools.cpp
    这是包含应用程序类 CTestToolsApp 的主应用程序源文件。

TestTools.rc
    这是程序使用的所有 Microsoft Windows 资源的列表。它包括 RES 子目录中存储的图标、位图和光标。此文件可以直接在 Microsoft Visual C++ 中进行编辑。项目资源包含在 2052 中。

res\TestTools.ico
    这是用作应用程序图标的图标文件。此图标包括在主资源文件 TestTools.rc 中。
...
TestToolsDlg.h、TestToolsDlg.cpp - 对话框
    这些文件包含 CTestToolsDlg 类。此类定义应用程序的主对话框的行为。对话框模板包含在 TestTools.rc 中，该文件可以在 Microsoft Visual C++ 中编辑。
