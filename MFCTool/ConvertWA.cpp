...
//秒变 时分秒 60 -> 00:01:00
std::string ConvertWA::SecToHMS(unsigned int sec)
{
	char str[20] = { 0 };

	int hour = sec / 3600;
	int mintue = sec / 60 % 60;
	int second = sec % 60;
	sprintf_s(str, "%02d:%02d:%02d", hour, mintue, second);
	return str;
}

unsigned int ConvertWA::HMSToSec(const string &hms)
{
	unsigned int num = 0;
	vector<string> list;
	SplitString(hms,list , ':');
	if (list.size() != 3)
	{
		return num;
	}

	num += atoi(list[0].c_str()) * 3600;
	num += atoi(list[1].c_str()) * 60;
	num += atoi(list[2].c_str());

	return num;
}

int ConvertWA::DateTimeSpanToSec(CString &startTime, CString &endTime)
{
	COleDateTime start_time;
	start_time.ParseDateTime(startTime);

	COleDateTime end_time;
	end_time.ParseDateTime(endTime);

	COleDateTimeSpan ts = end_time - start_time;

	//double dd = ts.m_span; //单位是天

	int seconds = ts.m_span * 24 * 3600;
