...
void MD5::Copy(MD5 * src)
{
	::memcpy(state, src->state, sizeof(state));
	::memcpy(count, src->count, sizeof(count));
	::memcpy(buffer, src->buffer, sizeof(buffer));
	::memcpy(digest, src->digest, sizeof(digest));
...
bool MD5::raw_digest(char * szOutput, int iLength)
{
	if(iLength <= 16 || !szOutput)
		return false;
	...
	memcpy((UCHAR *)szOutput, digest, 16);
	return true;
}

bool MD5::hex_digest(char * szOutput, int iLength)
{
	int i;

	if(iLength <= 32 || !szOutput)
		return false;
...
bool MD5::hex_digest16(char * szOutput, int iLength)
{
	int i;
	...
	for (i = 0; i < 16; i ++)
	{	sprintf(szOutput + i, "%1x", (digest[i] / 16) ^ (digest[i] % 16));
	}
